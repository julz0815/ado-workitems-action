...
export class WorkItemCreatoroAuth {
    ...
    private importParameters: CommonData.FlawImporterParametersDto;
                ...
                if (isSCAFlaw) {
                    ...
                    if (annotationResult.action === 'close' && isOpenState) {
                        ...
                        await this.updateWorkitemState(
                            workItemRef.id!,
                            this.importParameters.AdoCloseState || this.workItemStateClosed,
    ...
    async createWorkItems(workItemDetails: CommonData.workItemsDataDto) {
        ...
        if (await this.validateSupportedWITypes()) {
            ...
            for (let index = 0; index < workItemDetails.WorkItemList.length; index++) {
                ...
                let validWorkItemRefs: wi.WorkItemReference[] = [];
                if (workItemQueryResult.workItems && workItemQueryResult.workItems.length > 0) {
                    // Fetch full work item details to check project
                    const workItemIds = workItemQueryResult.workItems.map(wi => wi.id).filter((id): id is number => typeof id === "number");
    ...
    */
    ...
    async createWorkitem({ projectName, witype, title, description, severity, area, foundInBuild, tagsCollection, state, wiComments, overwriteAreaPathInWorkItemsOnImport, iterationPath, overwriteIterationPathInWorkItemsOnImport }: WorkItemParameters): Promise<number | null> {
        ...
        return this.manageCreateandUpdateWorkItem(
            wijson,
            projectName,
            witype,
            state,
            ...
            description,
            area,
            overwriteAreaPathInWorkItemsOnImport,
            wiComments || null,
            foundInBuild,
            iterationPath,
            overwriteIterationPathInWorkItemsOnImport
        );
    }
    ...
    async manageCreateandUpdateWorkItem(
        ...
        try {
            await this.vstsWI.createWorkItem(undefined, wijson, projectName, witype, undefined, undefined).then(async (workitem: wi.WorkItem) => {
                console.log(`WorkItem '${workitem.id}' Created.`);
                // Ensure description/repro steps are populated on creation
                if (workitem.id) {
                    try {
                        const currentRepro = (workitem.fields && workitem.fields["Microsoft.VSTS.TCM.ReproSteps"]) as string | undefined;
                        const currentDescription = (workitem.fields && workitem.fields["System.Description"]) as string | undefined;
                        const descValue = description || "";

                        const patchOps: vss.JsonPatchOperation[] = [];
     ...
     */
    private formatMitigation(annotation: any): { mitigation_title: string; mitigation: string } {
     ...
     */
    private processAnnotations(annotations: any[]): { action: string; annotations: any[]; mostRecent?: any } {
     ...
     */
    private async fetchWorkItemHistoryEntries(workItemId: number): Promise<string[]> {
            ...
            if (updates) {
                for (const upd of updates) {
                    ...
                    if (hist) {
                        ...
                        } else if (typeof (hist as any).newValue === 'string') {
                            allHistory.push((hist as any).newValue);
                        }
                    }
                }
            }
     ...
     */
    private async addMitigationComments(workItemId: number, annotations: any[]): Promise<void> {
        if (!annotations || annotations.length === 0) {
            return;
        }
        ...
        try {
            ...
            for (const annot of sortedAnnotations) {
                ...
                if (!existingSet.has(key)) {
                    const payload: vss.JsonPatchDocument = [
                        {
